...
(function() {
  ...
  async function activateSubscription(subscriptionCode) {
    ...
    try {
      ...
      // Fallback: modo offline com código local
      ...
      if (offlineResult.success) {
        state.subscription = {
          ...
          status: 'active',
          expiresAt: offlineResult.noExpiry ? null : null, // Sem expiração para master key
          trialEndsAt: null,
          activatedAt: new Date().toISOString(),
          lastSync: null,
        ...
        // Master key tem créditos ilimitados
      ...
      return { success: false, error: 'Erro ao validar código. Verifique sua conexão.' };
    }
  }
