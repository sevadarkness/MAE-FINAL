...
window.__whlSidepanelELM?.on(window, 'beforeunload', () => {
    try { window.__whlSidepanelELM.cleanup(); } catch (_) {}
});
      ...
      return `
        <div class="recover-item" data-id="${esc(msg.id || msg.key)}" style="padding:10px;border-bottom:1px solid rgba(255,255,255,0.1)">
          ...
          <div style="margin-top:6px;display:flex;gap:6px;flex-wrap:wrap">
            ...
            ${msg.action === 'edited' ? `<button class="recover-compare-btn sp-btn sp-btn-sm" data-id="${msg.id || msg.key}">📊 Comparar</button>` : ''}
            <button class="recover-download-btn sp-btn sp-btn-sm" data-id="${msg.id || msg.key}" data-has-media="${hasMedia ? 'true' : 'false'}">📥 Baixar</button>
          </div>
        </div>
      `;
    }).join('');
