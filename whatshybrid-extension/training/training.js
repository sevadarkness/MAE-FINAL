    ...
    if (type === 'executor') {
      ...
      content += `
        <div class="message-approval">
          <button class="btn-approve" onclick="app.approveMessage('${message.id}')">✅ Aprovar</button>
          <button class="btn-reject" onclick="app.rejectMessage('${message.id}')">❌ Rejeitar</button>
        </div>
      `;
    }
  ...
  async runQuickTest() {
    ...
    try {
      ...
      if (window.CopilotEngine) {
        ...
        contentDiv.innerHTML = '<span style="color: var(--danger);">Serviço de IA não disponível. Configure uma API Key nas configurações.</span>';
      }
    } catch (error) {
