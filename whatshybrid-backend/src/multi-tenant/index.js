...
function setupMultiTenant(app, db, options = {}) {
  ...
  tenantManager.init().then(() => {
    console.log('[MultiTenant] ✅ Sistema multi-tenant inicializado');
  });
  ...
  tenantManager.on('tenant:limit_reached', ({ tenantId, resource, limit }) => {
    console.warn(`[MultiTenant] ⚠️ Limite atingido: ${resource} para tenant ${tenantId} (${limit})`);
  });

  tenantManager.on('tenant:limit_warning', ({ tenantId, resource, remaining, limit }) => {
    console.warn(`[MultiTenant] ⚠️ Limite próximo: ${resource} para tenant ${tenantId} (${remaining}/${limit})`);
  });
